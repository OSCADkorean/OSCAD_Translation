...
<translation>
date : 2015.01.15
subject : EUPL-CA : 내장 소스코드로 수정 라이브러리 배포
...
"EUPL-1.1로 라이선스 된 받은 스니펫, 모듈, 라이브러리 또는 플러그인 등 스니몰리의 수정 버전을 소스코드 파일 형태 또는 소스코드 패키지로 제3자에게 내장 컴포넌트로서 스니펫, 모듈, 라이브러리 또는 플러그인을 포함하는 다른 더 큰 소프트웨어 유닛과 함께 배포하려는 의도를 갖고 있다.";
...
*/
	"라이선싱하는 요소, 특히 저작권, 특허 또는 상표 공지 그리고 라이선스와 보증 제한을 뜻하는 공지 등은 받았던 형태로 패키지에 포함한다.",
/*
		"Give the recipient a copy of the EUPL-1.1
		license. If it is not already part of the software package, add
		it.",
*/
		"받는 사람에게 EUPL-1.1 라이선스 사본을 제공한다. 이미 구성한 소프트웨어 패키지의일부에서 빠져있다면 이를 추가한다.",
/*
...
/*<수정사항 : EUPL -> EUPL-1.1>
		"Arrange your modifications of the embedded library in a way that they are
		covered by the existing EUPL-1.1 licensing statements. If you add new source
		code files into the scope of the library, insert a header containing
		your copyright line and an EUPL-1.1 adequate licensing
		the statement.",
*/
		"기존의 EUPL-1.1 라이선싱 사항에 포함되는 방식으로 수정사항을 구성한다. 만약 새로운 소스코드 파일을 추가했다면, 헤더에 여러분의 저작권을 명시한 문장과 그 문장이 EUPL-1.1을 충족시키도록 라이선싱한다는 내용을 추가한다.",
/*
...
/*		
		...
		thoroughly within the source code and include the date of the
		modification."
*/
...
"배포 문서 및 추가 자료를 통해 또한 기존의 저작권 공지 텍스트 파일 내용, 소프트웨어 명칭에 대한 힌트, 홈페이지 링크, EUPL-1.1 라이선스 링크를 복제한다. 특히, 자체저작권공지의 하위 구역으로써 포함한다."
);
