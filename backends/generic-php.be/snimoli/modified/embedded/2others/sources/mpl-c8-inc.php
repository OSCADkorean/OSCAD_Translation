...
$gFbLiSpUcDesc=
	"MPL로 라이선스 된 받은 스니펫, 모듈, 라이브러리 또는 플러그인 등 스니몰리의 수정 버전을 소스코드 파일 형태 또는 소스코드 패키지로 제3자에게 내장 컴포넌트로서 스니펫, 모듈, 라이브러리 또는 플러그인을 포함하는 다른 더 큰 소프트웨어 유닛과 함께 배포하려는 의도를 갖고 있다.";
...
$gFbLiSpUcRequiresManArr=array(
	...
	"받는 사람에게 MPL 2.0 라이선스 사본을 제공한다. 이미 구성한 소프트웨어 패키지의 일부에서 빠져있다면 이를 추가한다.566 패키지의 라이선싱 파일의 라이선싱 설명에서 MPL에 의거 패키지를 사용 허가하였다는 점이 명시되지 않으면 다음 문장이 들어간 자체 올바른 MPL 라이선싱 파일을 추가로 삽입한다. 이 소스코드 형태는 Mozilla Public License 버전 2.0 조항에 준한다. MPL 사본이 이 파일과 함께 배포되지 않았다면http://mozilla.org/MPL/2.0/에서 사본 하나를 확보할 수 있다.",
	"기존의 MPL 라이선싱 사항에 포함되는 방식으로 내장 라이브러리의 수정물을 구성한다. 만약 새로운 소스코드 파일을 라이브러리 자체에 추가했다면, 헤더에 여러분의 저작권을 명시한 문장과 그 문장이 MPL을 충족하도록 라이선싱한다는 내용을 추가한다."
...
$gFbLiSpUcRequiresVolArr=array(
	"통합된 MPL과 라이선싱 파일이 내장 라이브러리만을 명확히 뜻하고 자신의 지배적인 저작물의 라이선싱을 침해하지 않도록 소스코드 배포물을 구성한다. 추가 라이선싱하는 요소가 전부 포함된 특정 디렉토리에 라이브러리, 모듈, 스니펫 또는 플러그인과 같은 삽입된 컴포넌트를 두는 것이 바람직하다.",
