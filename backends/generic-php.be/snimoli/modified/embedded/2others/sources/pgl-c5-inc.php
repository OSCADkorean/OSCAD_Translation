...
$gFbLiSpUcDesc=
	"PGL로 라이선스 된 받은 스니펫, 모듈, 라이브러리 또는 플러그인 등 스니몰리의 수정 버전을 바이너리 형태 또는 소스코드 파일로 제3자에게 내장 컴포넌트로 스니펫, 모듈, 라이브러리 또는 플러그인을 포함하는 다른 더 큰 소프트웨어 유닛과 함께 배포하려는 의도를 갖고 있다.";
...
$gFbLiSpUcRequiresVolArr=array(
	...
	"실행 프로그램에 의해 표시되는 저작권 공지에서는 PGL 라이선스에 기반하여 사용 허가된 컴포넌트를 프로그램이 사용한다는 설명을 밝히는 것이 오픈소스 커뮤니티에서는 바람직한 행위이다. 그리고 이 사용된 컴포넌트의 홈페이지/다운로드페이지 링크를 삽입하는 것이 바람직하다.",
	"또한 프로그램 문서 및 추가 자료에 초기 소프트웨어 컴포넌트와 그 홈페이지 링크로 추가한 이 컴포넌트를 사용하였다는 점을 언급하는 것이 바람직하다.",
	"초기 라이선싱하는 요소, 특히 원저작자 특정 저작권 공지, 허가 공지 및 PGL 권리포기 사항이 들어있는 PGL 라이선스 텍스트가 내장 라이브러리만을 확실히 뜻하고 자신의 지배적인 저작물의 라이선싱을 침해하지 않도록 배포물을 구성한다. 모든 라이선싱하는 요소가 들어간 특정 디렉토리에 라이브러리, 모듈, 스니펫 또는 플러그인을 두는 것이 바람직하다."
