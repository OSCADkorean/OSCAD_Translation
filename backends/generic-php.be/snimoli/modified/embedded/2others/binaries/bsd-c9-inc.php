...
$gFbLiSpUcDesc=
"BSD로 라이선스 된 받은 스니펫, 모듈, 라이브러리 또는 플러그인 등 스니몰리의 수정 버전을 바
...
$gFbLiSpUcRequiresManArr=array(
		"배포물에 받았던 형태로 최초 저작권 공지, BSD 라이선스, BSD 권리포기사항을 포함한다. 소스코드 패키지에 기반하여 바이너리 파일을 컴파일하고 이 컴파일로 라이선싱 파
...
실행 프로그램에 의해 표시되는 것이 바람직하다. 왜냐하면 더 큰 소프트웨어 유닛에 이
스니몰리를 삽입하는 것은 더 큰 유닛을 개발하는 것이기 때문이다. 따라서 BSD 컴포넌
트의 힌트로 더 큰 유닛의 저작권 공지를 확장할 수도 있다.",

  "라이선싱하는 요소들 특히, BSD 라이선스 텍스트, 원저작자 특정 저작권 공지, BSD 권리
포기는 자신의 지배적인 저작물의 라이선싱을 침해하지 않고 내장 라이브러리만을 명확
히 나타내도록 배포할 소스코드를 구성한다. 모든 라이선싱하는 요소가 들어있는 특정
디렉토리에 라이브러리, 모듈, 스니펫 또는 플러그인을 두는 것이 바람직하다."
);
