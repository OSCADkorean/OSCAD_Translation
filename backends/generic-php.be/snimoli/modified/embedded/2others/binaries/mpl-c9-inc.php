...
$gFbLiSpUcDesc=
	"MPL2.0으로 라이선스 된 받은 스니펫, 모듈, 라이브러리 또는 플러그인의 수정 버전을 바이너리 파일
...
게 한다. 인터넷 저장소에 소스코드 패키지를 삽입하고 다운로드하는 사용자로부터 수수
료를 요구하지 않고 다운로드 기능을 허용한다. 코드를 수신하는 방법을 설명하는 배포
패키지에 손쉽게 찾는 설명을 포함한다. 이 저장소는 충분히 오랫동안 사용할 수 있도록
한다.",
		...
		"기존의 MPL2.0 라이선싱 사항에 포함되는 방식으로 내장 라이브러리의 수정물을 구성한다."
);
...
일부에서 빠져있다면 이를 추가한다.569 패키지의 라이선싱 파일의 라이선싱 설명에서
...
http://mozilla.org/MPL/2.0/에서 사본 하나를 확보할 수 있다.",
		...
		"통합된 MPL과 라이선싱 파일이 내장 라이브러리만을 명확히 뜻하고 자신의 지배적인 저
작물의 라이선싱을 침해하지 않도록 바이너리 배포물을 구성한다. 추가 라이선싱하는 요
소가 전부 포함된 특정 디렉토리에 라이브러리, 모듈, 스니펫 또는 플러그인과 같은 삽
입된 컴포넌트를 두는 것이 바람직하다.",

		"배포 문서 및 추가 자료를 통해 또한 기존의 저작권 공지 텍스트 파일 내용, 사용된
MPL 라이선스 컴포넌트의 명칭 힌트, 홈페이지 링크, MPL 2.0 라이선스 링크를 복제한
다(특히, 자체 저작권 공지의 서브섹션으로서)."
);
