...
binary files or as unmodified binary package. In this case it does not matter if
...
"LGPL-2.1 및 LGPL-3.0로 라이선스 된 받은 소프트웨어의 비수정 버전을 바이너리 파일 형태 또는 바이너리 패키지
...
/*  "Make the source code of the distributed software
  ...
  "자신의 권한으로 배포된 소프트웨어는 이를 수정하지 않았더라도 소스코드를 저장소(repository)를 통해 이용할 수 있게 한다. 인터넷 저장소에 소스코드 패키지를 삽입하고, 인터넷을 통한 다운로드를 허용한다. 코드를 수신하는 방법을 설명하는 배포 패키지에 손쉽게 찾을 수 있는 설명을 포함시킨다. 이 저장소는 소프트웨어 패키지 실체를 배포한 후 적어도 3년 동안 온라인에서 사용할 수 있도록 한다.",
...
/*  "Retain all existing copyright notices."*/
	"기존의 모든 저작권 공지와 라이선신하는 요소를 유지한다."
