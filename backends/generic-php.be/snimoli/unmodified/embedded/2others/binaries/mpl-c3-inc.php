...
/*	"that you received MPL-2.0 licensed software which you are now
  ...
  "MPL-2.0으로 라이선스 된 받은 소프트웨어의 비수정 버전을 바이너리 형태 또는 바이너리 패키지로 제3자에게 배포하고자 하는 의도를 갖고 있다. 이 경우 독립 또는 내장 유닛으로 프로그램, 어플리케이션, 서버, 스니펫, 모듈, 라이브러리 또는 플러그인 등 배포유형을 식별하는 것은 아니다.";
...
/*  "Ensure that the licensing elements - esp. all
  ...
  "라이선싱하는 요소, 특히 모든 저작권 공지, 특허 공지, 보증 제한 또는 책임 배제는 받았던 형태로 정확히 패키지에 포함한다. 소스에서 바이너리를 컴파일한다면 라이선싱하는 요소가 패키지에 전부 포함되도록 한다.",
...
/*  "Make the source code of the software accessible
  ...
  "자신의 권한으로 배포된 소프트웨어의 소스코드를 저장소(repository)를 통해 이용할 수 있게 한다. 인터넷 저장소에 소스코드 패키지를 삽입하고 다운로드하는 사용자로부터 수수료를 요구하지 않고 다운로드 기능을 허용한다. 코드를 수신하는 방법을 설명하는 배포 패키지에 손쉽게 찾을 수 있는 설명을 포함시킨다. 이 저장소는 충분히 오랫동안 사용할 수 있도록 한다.",

/*  "Insert a prominent hint to the download repository
  into your distribution and/or your additional material.",*/
  "배포 및 추가 자료에 다운로드 저장소의 힌트를 눈에 띄게 삽입한다.",

/*  "Execute the to-do list of use case MPL-2.0-C2 (:- the same request parameters,
		 but replace form = binaries by form = sources)."*/
	"사용 사례 MPL-2.0-C2의 과업 목록을 실행한다."
...
/*  "Give the recipient a copy of the MPL-2.0 license. If it is not already
  ...
  "받는 사람에게 MPL-2.0 라이선스 사본을 제공한다. 이미 구성한 소프트웨어 패키지의 일부에서 빠져있다면 이를 추가한다. 패키지의 라이선싱 파일의 라이선싱 설명에서 MPL-2.0에 의거, 패키지를 사용 허가하였다는 점이 명시되지 않으면 다음 문장이 들어간 자체 올바른 MPL-2.0 라이선싱 파일을 추가로 삽입한다: '이 소스코드 형태는 Mozila Public License 버전 2.0 조항에 준한다. MPL 사본이 이 파일과 함께 배포되지 않았다면, htp:/mozila.org/MPL/2.0/에서 사본 하나를 확보할 수 있다.'.",
