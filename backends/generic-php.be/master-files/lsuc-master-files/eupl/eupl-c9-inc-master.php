...
$gFbLiSpUcDesc=
	"EPL로 라이선스 된 받은 스니펫, 모듈, 라이브러리 또는 플러그인의 수정 버전을 바이너리 파일 형
태 또는 바이너리 패키지로 제3자에게 내장 컴포넌트로서 스니펫, 모듈, 라이브러리 또는 플러그인
을 포함하는 다른 더 큰 소프트웨어 유닛과 함께 배포하려는 의도를 갖고 있다.";
...
게 한다. 인터넷 저장소에 소스코드 패키지를 삽입하고 그 다운로드 기능을 허용한다. 코드를 수신하는 방법을 설명하는 배포 패키지에 손쉽게 찾는 설명을 포함한다. 이 저장
소는 충분히 장기간 사용할 수 있도록 한다. "

" 배포 및 추가 자료에 다운로드 저장소의 눈에 띄는 힌트를 삽입한다. "

" 사용 사례 EPL-C8의 과업 목록을 실행한다."
 
" 통합된 EPL과 라이선싱 파일이 자신이 보유한 지배적인 저작물의 라이선싱을 침해하지
않고 내장 라이브러리만을 명확히 나타내도록 바이너리(binary distribution)를 구성한다. 모든 라이선싱하는 요소가 추가로 들어있는 특정 디렉토리에 라이브러리, 모듈, 스니펫
