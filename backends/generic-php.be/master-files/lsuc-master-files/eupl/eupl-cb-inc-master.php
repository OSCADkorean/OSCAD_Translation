...
*/
    " EUPL-1.1로 라이선스 된 받은 스니펫, 모듈, 라이브러리 또는 플러그인 등 스니몰리의 수정 버전을 바이너리 파일 형태 또는 바이너리 패키지로 제3자에게 내장 컴포넌트로서 스니펫, 모듈, 라이브러리 또는 플러그인을 포함하는 다른 더 큰 소프트웨어 유닛과 함께 배포하려는 의도를 갖고 있다. ";
...
*/
    "라이선싱하는 요소, 특히 저작권, 특허 또는 상표 공지 그리고 라이선스와 보증 제한을 뜻하는 공지 등은 받았던 형태로 패키지에 포함한다. 소스에서 바이너리를 컴파일한다면 라이선싱하는 요소는 패키지에 전부 포함한다.",
/*
...
/*
		...
		the software.",
*/
    "자신의 권한으로 내장 라이브러리의 소스코드와 지배적인 프로그램의 소스코드를 저장소(repository)를 통해 이용할 수 있게 한다. 인터넷 저장소에 소스코드 패키지를 삽입하고 인터넷을 통한 다운로드를 허용한다. 코드를 수신하는 방법을 설명하는 배포 패키지에 손쉽게 찾는 설명을 포함한다. 이 저장소는 소프트웨어 배포를 지속하는 동안 충분한 기간 동안 온라인에서 사용할 수 있도록 한다.",
/*
...
/*
		"Arrange your modifications of the embedded library in a way that they
		are covered by the existing EUPL-1.1 licensing statements.",
*/
    "기존의 EUPL-1.1 라이선싱 사항에 포함되는 방식으로 삽입된 라이브러리의 수정사항을 구성 한다.",
/*
		"License your overarching program also under the EUPL-1.1.",
*/
    "사용 사례 EUPL-CA의 과업 목록을 실행한다",
/*
		"Execute the to-do list of use case EUPL-1.1-CA (:- the same request parameters,
...
/*
	"Mark all modifications of source code of the
		embedded library (snimoli) thoroughly within the source code and include the date of the
  modification.",
*/
    "내장 라이브러리(스니몰리)의 소스코드의 수정사항을 소스코드 안에 그리고 수정 날짜를 포함하여 전부 표시한다.",
/*
