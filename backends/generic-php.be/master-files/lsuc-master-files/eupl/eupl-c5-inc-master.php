...
$gFbLiSpUcDesc=
	"EUPL로 라이선스 된 받은 스니펫, 모듈 또는 라이브러리의 비수정 버전을 더 큰 유닛의 내장 컴포
넌트로, 바이너리 파일 형태 또는 바이너리 패키지로 제3자에게 배포하려는 의도를 갖고 있다. ";
...
뜻하는 공지 등은 받았던 형태로 패키지에 포함한다. 소스에서 바이너리를 컴파일한다면 라이선스 요소는 전부 패키지에 포함하도록 한다.",

		"받는 사람에게 EUPL-1.1 라이선스 사본을 제공한다. 이미 구성한 바이너리 패키지의 일
부에서 빠져있다면 이를 추가한다",

		"자신의 권한으로 내장 라이브러리의 소스코드 그리고 지배적인 프로그램의 소스코드를
저장소(repository)를 통해 이용할 수 있게 한다(이를 수정하지 않더라도). 인터넷 저장소
에 소스코드 패키지를 삽입하고 인터넷을 통한 다운로드를 허용한다. 코드를 수신하는
방법을 설명하는 배포 패키지에 손쉽게 찾는 설명을 포함한다. 이 저장소는 소프트웨어
배포를 지속하는 동안 충분한 기간 동안 온라인에서 사용할 수 있도록 한다.",

		"배포 및 추가 자료에 다운로드 저장소의 힌트를 눈에 띄게 삽입한다.",

		" EUPL-1.1에 의거 지배적인 프로그램의 사용을 허가한다. EUPL-1.1 라이선싱 사항에 포함
되는 방식으로 기능 추가 개발물의 바이너리를 구성한다.",

		"사용 사례 EUPL-C4의 과업 목록을 실행한다"
);
...
명칭에 대한 힌트, 홈페이지 링크, EUPL-1.1 라이선스 링크를 복제한다"
);
