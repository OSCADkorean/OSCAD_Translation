...
$gFbLiSpUcDesc=
	"EPL로 라이선스 된 받은 프로그램, 어플리케이션 또는 서버 등 프로앱스의 수정 버전을 바이너리 파일 형태 또는 바이너리 패키지로 제3자에게 배포하려는 의도를 갖고 있다.";
...
정확한 형태로 패키지에 포함한다. 소스코드에서 바이너리를 컴파일한다면 라이선싱하는
요소가 패키지에 전부 포함되도록 한다.",
...
시되는 기존의 저작권 화면을 업데이트한다.",
  ...
  "자신의 권한으로 소프트웨어의 소스코드를 저장소(repository)를 통해 이용할 수 있게 한
다. 인터넷 저장소에 소스코드 패키지를 내보내고 그 다운로드 기능을 허용한다. 코드를
수신하는 방법을 설명하는 내용을 배포 패키지에 손쉽게 찾도록 하여 포함한다. 이 저장
소는 충분히 장기간 사용할 수 있도록 한다.",

  "배포 및 추가 자료에 다운로드 저장소의 눈에 띄는 힌트를 삽입한다.",

  "사용 사례 EPL-C4의 과업 목록을 실행한다."
