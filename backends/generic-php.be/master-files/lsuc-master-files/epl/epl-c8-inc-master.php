...
*/
	...
	"EPL-1.0으로 라이선스 된 받은 스니펫, 모듈, 라이브러리 또는 플러그인 등 스니몰리의 수정 버전을 소스코드 파일 형태 또는 소스코드 패키지로 제3자에게 내장 컴포넌트로서 스니펫, 모듈, 라이브러리 또는 플러그인을 포함하는 다른 더 큰 소프트웨어 유닛과 함께 배포하려는 의도를 갖고 있다.";
...
*/
		...
		"EPL-1.0 라이선스 사본을 받는 사람에게 제공한다. 이미 구성한 소프트웨어 패키지의 일부에서 빠져있다면 이를 추가한다.375 패키지의 라이선싱 파일 내 라이선싱 설명이 패키지가 EPL-1.0에 준한다는 설명이 명확하지 않으면, 자체적으로 정확한 EPL-1.0라이선싱 파일을 추가로 삽입한다.",
...
*/
		...
		"만약 배포된 소스코드 패키지 안에 존재하지 않는다면, 매우 두드러지게 ‘보증 없음’이란 명확한 문구를 포함시킨다. 이 문구로 모든 소프트웨어 기여자에게 소프트웨어 품질에 대한 책임을 전가하지 않는다는 점을 명확히 한다. 그 다음에 그 파일에 EPL-1.0법적 책임의 권리포기와 보증 없음 조항을 추가한다. 내장 컴포넌트 스펙으로서 동일한 정보를 자체의 지배적인 프로그램의 저작권 화면에 표시한다.",
...
*/
	...
	"기존의 EPL-1.0라이선싱 문구로 적용되는 방식으로 내장 라이브러리의 수정물을 구성한다. 만약 새로운 소스코드 파일을 추가했다면, 헤더에 여러분의 저작권을 명시한 문장과 그 문장이 EPL-1.0을 충족시키도록 라이선싱한다는 내용을 추가한다.",
