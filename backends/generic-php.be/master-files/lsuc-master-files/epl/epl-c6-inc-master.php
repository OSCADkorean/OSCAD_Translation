...
*/
	...
	"EPL-1.0으로 라이선스 된 받은 스니펫, 모듈, 라이브러리 또는 플러그인 등 스니몰리의 수정 버전을 소스코드 파일 형태 또는 소스코드 패키지로 제3자에게 배포하려는 의도를 갖고 있다. 단, 이를 다른 더 큰 소프트웨어 유닛에 삽입하지는 않는다.";
...
*/
	...
	"받는 사람에게 EPL-1.0 라이선스 사본을 제공한다. 이미 구성한 소프트웨어 패키지의 일부에서 빠져있다면 이를 추가한다. 패키지의 라이선싱 파일 내 라이선싱 문구에서 패키지가 EPL에-1.0준한다는 설명이 명확하지 않으면 추가로 정확한 EPL-1.0 라이선싱 파일을 삽입한다.",
