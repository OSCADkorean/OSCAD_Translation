...
*/
"PHP로 라이선스 된 받은 프로그램, 어플리케이션 또는 서버의 수정 버전을 (proapse) 소스코드 파일 형태 또는 소스코드 패키지로 제3자에게 배포하려는 의도를 갖고 있다.";
...
"전체 PHP 라이선스(특히, 저작권 공지, PHP 조건 및 PHP 권리포기)는 받았던 형태로 패키지에 포함한다.",
  ...
  "배포 문서 및 추가 자료에 “이 제품은 다음의 주소(htp:/www.php.net/)에서 자유로이이용할 수 있는 PHP를 포함한다”는 형태로 승인 문구를 포함한다."
);
...
"배포 문서 및 추가 자료에 또한 초기 저작권 공지, PHP 조건 및 PHP 권리포기사항을 포함한다",
	...
	"실행 프로그램에 의해 표시되는 저작권 공지에서는 PHP 라이선스에 기반하여 프로그램이 사용 허가되었다는 설명을 밝히는 것이 오픈소스 커뮤니티에서는 바람직한 행위이다. 왜냐하면, 프로그램을 수정하고 있기 때문에 초기 저작권 공지에 그 설명이 결여되었다면 그런 힌트를 추가할 수 있다. 저작권 화면에 그런 공지가 누락되었다면 저작권 공지, PHP 조건 및 PHP 권리포기사항을 포함하여 전체 PHP 라이선스를 복제할 수 있는지 고려한다(바이너리 배포 시 요구됨).",
