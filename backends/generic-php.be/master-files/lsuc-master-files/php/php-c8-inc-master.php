...
# Editor : Buyong Shin <tlsqndyd@gmail.com>
...
/*
  ...
  component.";*/
"PHP로 라이선스 된 받은 스니펫, 모듈, 라이브러리 또는 플러그인 등 스니몰리의 수정 버전을 소스코드 파일 형태 또는 소스코드 패키지로 제3자에게 내장 컴포넌트로서 스니펫, 모듈, 라이브러리 또는 플러그인을 포함하는 다른 더 큰 소프트웨어 유닛과 함께 배포하려는 의도를 갖고 있다.";
...
*/
"전체 PHP 라이선스(특히, 저작권 공지, PHP 조건 및 PHP 권리포기)는 받았던 형태로 패키지에 포함한다",

  "배포 문서 및 추가 자료에 “이 제품은 다음의 주소(htp:/www.php.net/)에서 자유로이이용할 수 있는 PHP를 포함한다”라는 형태로 승인 문구를 포함한다.",
  
  "배포 문서 및 추가 자료에 또한 초기 저작권 공지, PHP 조건 및 PHP 권리포기사항을 포함한다."
		);
...
*/
"실행 프로그램에 의해 표시되는 저작권 공지에서는 PHP 라이선스에 기반하여 사용 허가된 컴포넌트를 프로그램이 사용한다는 설명을 밝히는 것이 오픈소스 커뮤니티에서는 바람직한 행위이다. 지배적인 프로그램의 저작권 화면을 통해 저작권 공지, PHP 조건 및PHP 권리포기사항을 포함하여 전체 PHP 라이선스를 복제한다(바이너리 배포 시 요구됨).6",

  "소스코드 안에 수정사항을 표시한다",

  "라이선싱하는 요소(특히, PHP 라이선스 텍스트, 원저작자 특정 저작권 공지 및 PHP 권리포기사항)가 내장 라이브러리만을 명확히 가리키고 자신의 지배적인 저작물의 라이선싱을 침해하지 않도록 소스코드 배포물을 구성한다. 모든 라이선싱하는 요소가 들어간 특정 디렉토리에 라이브러리, 모듈, 스니펫 또는 플러그인과 같은 내장 컴포넌트를 두는 것이 바람직하다"
);
